...
def get_slot_map():
    ...
    t_start = 8.5
    idx = 0
    while t_start <= 19.5: # ขยายเพิ่มเพื่อรองรับ End time ของคาบสุดท้าย
...
# ==========================================
# 3. Solver Engine (Python 3.13 Compatible)
# ==========================================
def calculate_schedule(files, mode, solver_time, penalty_val):
    ...
    try:
        ...
        for _, row in df_courses.iterrows():
            ...
            tea, opt = t_map.get(c, ['Unknown']), row.get('optional', 1)
            ...
            for r in room_list:
                ...
                for di in range(5):
                    for si in range(TOTAL_SLOTS - t['dur']):
                        ...
                        if any(tid in un_map and si+i in un_map[tid][di] for tid in t['tea'] for i in range(t['dur'])): continue
        ...
        if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
            for t in (fixed_tasks + tasks):
                ...
                if solver.Value(is_sched[uid]):
                    ...
                    # แก้ไขการดึงห้องแบบ Safe (Python 3.13 Compatible)
                    room_name = "Unknown"
                    for k, v in vars.items():
                        if k[0] == uid and k[2] == d and k[3] == s and solver.Value(v):
                            room_name = k[1]; break
                    res_final.append({'Day': DAYS[d], 'Start': SLOT_MAP[s]['time'], 'End': SLOT_MAP[s+t['dur']]['time'], 'Room': room_name, 'Course': t['id'], 'Sec': t['sec'], 'Type': t.get('type','-'), 'Teacher': ",".join(t['tea']), 'Note': "Ext.Time" if (SLOT_MAP[s]['val'] < 9.0 or SLOT_MAP[s+t['dur']-1]['val'] >= 16.0) else ""})
